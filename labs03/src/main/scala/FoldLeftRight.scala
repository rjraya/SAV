...
object FoldLeftRight {
  ...
  lazy val theorem: Theorem =
    forallI("z" :: A, "f" :: ((A, A) =>: A),"l"::ListA) { case (z,f,l) =>
      implI(isMonoid(z, f)) { isMonoid =>
        ...
        def lemmaProperty(l: Expr) = {
          forall("x" :: A){ case x =>
            foldl(f, x, l) === f(x, foldl(f, z, l))
          }
        }
        val lemma: Theorem = structuralInduction(lemmaProperty _, "l" :: ListA) { case (ihs, goal) =>
        ...
        def property(l: Expr) = {
            foldl(f, z, l) === foldr(f,z,l)
        }

        val lemma1: Theorem = structuralInduction(property _, "l" :: ListA) { case (ihs, goal) =>
          ihs.expression match {
            case C(`cons`, h, t) =>
